...
**Default deployment target:** courses are generated in a format directly compatible with the user's self-hosted **Courses Hub** module in the [pi-automate](https://github.com/Jarkendar/pi-automate) repo. The file drops straight into `courses_src/` and the existing patcher will handle adapter injection without modification.
...
## Storage Format Contract (Courses Hub compatibility)

The generated HTML must use **exactly** this storage shape so the hub's patcher and adapter recognize it without code changes:

### Required JS constants

```js
// Storage key - must match `course_<slug>_v1` convention
const COURSE_KEY = "course_<slug>_v1";

// Top-level course object - patcher looks for `const COURSE = {`
const COURSE = {
  title: "<Polish title>",
  modules: [
    {
      ...
      lessons: [
        {
          id: "<unique-lesson-slug>",   // e.g. "compose-state-hoisting"
          title: "<Lesson title>",
          description: "<4-6 sentences>",
          tags: ["tag1", "tag2"],
          method: "tdd-loop",            // ID from Teaching Methods Library
          check: "code-challenge",       // ID from Knowledge Check Methods
          time: "60 min"
        }
      ]
    }
...
### Required localStorage shape

```js
function loadState() {
  const raw = localStorage.getItem(COURSE_KEY);
  return raw ? JSON.parse(raw) : { status: {}, notes: {} };
}
...
**State object:** `{ status: { [lessonId]: "completed"|"in_progress"|"not_started" }, notes: { [lessonId]: "..." } }`

**Why this exact shape:** the Courses Hub adapter has per-course callbacks (`readLessons` / `writeLessons` / `lessonCount`) generated by the patcher based on this structure. Deviating breaks sync without warning.

### Lesson ID rules

- Use **slug-style explicit IDs** (`compose-state-hoisting`, `mvi-pattern`), not generated indices
- IDs must be **stable** — never renumber across course revisions, or progress maps will desync
- Keep them URL-safe (lowercase, hyphens, no special chars)
...
Each lesson has:
- **id** — slug, stable, URL-safe (see Storage Format Contract)
...
**Save the file to** `/mnt/user-data/outputs/course-<slug>.html`

The filename slug must:
- Match the `<slug>` used in `COURSE_KEY = "course_<slug>_v1"`
- Be lowercase, hyphenated, no special characters
- Mirror the topic concisely (e.g. `course-jetpack-compose.html`, `course-rust-basics.html`)

This keeps it consistent with hub conventions — files in `courses_src/` are named `course-<slug>.html` and the patcher derives the course_id from the filename.
...
**Default: localStorage in the exact shape defined in Storage Format Contract above.**

The generated HTML works standalone (opens directly in a browser, persists to `localStorage`). When dropped into the user's Courses Hub (`courses_src/` → `make courses-patch`), the existing patcher automatically wraps the same `localStorage` reads with the hub's sync adapter — no code changes needed because the format matches what the adapter expects.

Do NOT manually inject `<script src="courses-sync.js">` or `CoursesSync.attach()` into the generated HTML. The patcher handles this. Pre-injecting risks duplication or version mismatch.
...
- Status persisted via `saveState()` → `state.status[lessonId] = "completed"`
...
All state is persisted via `loadState()` / `saveState()` to the single key `COURSE_KEY` in the shape `{ status: {}, notes: {} }`. State survives page refresh, is browser-local until synced via Courses Hub.

**Status changes** — each lesson has a 3-state selector. When state changes:
- Lesson card gets the appropriate style (`done` / `in-progress`)
- Module mini progress bar updates
- Global progress bar updates
- Module border turns green when all lessons done

**Buttons:**
- "↺ Reset Progress" — clears all statuses (confirms first), keeps notes intact
...
- "Save" button — writes to `state.notes[lessonId]`, shows `saved ✓` confirmation for 2s
...
→ Skill triggers, generates `/mnt/user-data/outputs/course-jetpack-compose.html` with:
- `const COURSE_KEY = "course_jetpack-compose_v1"`
- `const COURSE = { title: "Jetpack Compose", modules: [...] }`
- ~4–5 modules starting from Compose fundamentals (skipping Android basics)
- Progressing through state management, theming, animations, testing
- `tdd-loop` and `project-based` methods dominant
- `build-test` and `debug-challenge` as checks
- localStorage shape `{ status: {}, notes: {} }`

The file drops straight into `courses_src/`, gets patched by the hub, becomes available at `/dashboard/courses/courses/course-jetpack-compose.html` with progress synced to the central API.